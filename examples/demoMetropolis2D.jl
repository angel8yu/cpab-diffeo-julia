...
function leastSquaresTransformation(θ)
  computeAndEvaluateTransformationSingleThread(B,θ,As,pts_src1,pts_fwd1,solutionSingleThread1,t,nCols,nRows,incX,incY,Nsteps,Δt,nSteps,ΔtAs,expT,Avees,bdryConstraints)
end

iterations = 50000
using PyPlot
println("Running Metropolis")
@time optimizedθ, acceptanceTrace, llTrace, lpTrace, lpllTrace = metropolis(leastSquaresTransformation, rand(size(θ,1)), iterations, 10.0, priorCov, 0.00001*priorCov, true)
println("Done!")
optimizedθArray = convert(Array, optimizedθ)

PyPlot.figure("Metropolis",figsize=(18,6))
